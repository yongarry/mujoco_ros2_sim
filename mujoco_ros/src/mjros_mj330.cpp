...
void profilerupdate(void)
{
    ...
    figconstraint.linepnt[0] = mjMIN(mjMIN(*d->solver_niter, mjNSOLVER), mjMAXLINEPNT);
    for (i = 1; i < 5; i++)
    ...
    // update cost figure
    figcost.linepnt[0] = mjMIN(mjMIN(*d->solver_niter, mjNSOLVER), mjMAXLINEPNT);
    for (i = 1; i < 3; i++)
...
void infotext(char *title, char *content, double interval)
{
    ...
    if (*d->solver_niter)
    {
        int ind = mjMIN(*d->solver_niter - 1, mjNSOLVER - 1);
    ...
    sprintf(content, "%-20.5f\n%-20.5f\n%-20.5f\n%d  (%d con)\n%.3f\n%.1f  (%d it)\n%.0f\n%.3f\n%.3f\n%.3f\n%s\n%-20.6f",
            ...
            solerr, *d->solver_niter,
            1 / interval,
            d->maxuse_stack / (double)d->pstack,
...
void makephysics(int oldstate)
{
    ...
    mjuiDef defPhysics[] = {
        ...
        {mjITEM_EDITINT,   "Iterations",    2, &(m->opt.iterations),        "1 0 1000"},
        {mjITEM_EDITNUM,   "Tolerance",     2, &(m->opt.tolerance),         "1 0 1"},
        {mjITEM_EDITINT,   "LS Iter",       2, &(m->opt.ls_iterations),     "1 0 100"},
        {mjITEM_EDITNUM,   "LS Tol",        2, &(m->opt.ls_tolerance),      "1 0 0.1"},
        {mjITEM_EDITINT,   "Noslip Iter",   2, &(m->opt.noslip_iterations), "1 0 1000"},
        {mjITEM_EDITNUM,   "Noslip Tol",    2, &(m->opt.noslip_tolerance),  "1 0 1"},
        {mjITEM_EDITINT,   "CCD Iter",      2, &(m->opt.ccd_iterations),    "1 0 1000"},
        {mjITEM_EDITNUM,   "CCD Tol",       2, &(m->opt.ccd_tolerance),     "1 0 1"},
        {mjITEM_EDITNUM,   "API Rate",      2, &(m->opt.apirate),           "1 0 1000"},
        {mjITEM_EDITINT,   "SDF Iter",      2, &(m->opt.sdf_iterations),    "1 1 20"},
        {mjITEM_EDITINT,   "SDF Init",      2, &(m->opt.sdf_initpoints),    "1 1 100"},
      ...
      mjuiDef defOverride[] = {
        ...
        {mjITEM_EDITNUM,   "Sol Imp",       2, &(m->opt.o_solimp),          "5"},
        {mjITEM_EDITNUM,   "Sol Ref",       2, &(m->opt.o_solref),          "2"},
        {mjITEM_EDITNUM,   "Friction",      2, &(m->opt.o_friction),        "5"},
        {mjITEM_END}
    };
    // add physics
    ...
    mjuiDef defRendering[] =
        {
            ...
            {mjITEM_SELECT, "Frame", 2, &(vopt.frame),
             "None\nBody\nGeom\nSite\nCamera\nLight\nWorld"},
            {mjITEM_BUTTON, "Copy camera", 2, NULL, ""},
...
void makesections(void)
{
    ...
    std::cout << "problem here?5"<<std::endl;
}
